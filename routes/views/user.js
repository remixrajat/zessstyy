...
module.exports = {
    ...
    listFood: async (req, res) => {
        ...
        let mon = {Breakfast: [], Lunch: [], Dinner: []};
        let tue = {Breakfast: [], Lunch: [], Dinner: []};
        let wed = {Breakfast: [], Lunch: [], Dinner: []};
        let thur = {Breakfast: [], Lunch: [], Dinner: []};
        let fri = {Breakfast: [], Lunch: [], Dinner: []};
        let sat = {Breakfast: [], Lunch: [], Dinner: []};
        let sun = {Breakfast: [], Lunch: [], Dinner: []};
        
        for await(let element of breakfast){
            console.log("loop1");
            newElem = element.available_days;
            newElem.map((i)=>{
                if(i.name == 'Monday'){
                    mon.Breakfast.push(element) 
                }
                if(i.name == 'Tuesday'){
                    tue.Breakfast.push(element) 
                }
                if(i.name == 'Wednesday'){
                    wed.Breakfast.push(element) 
                }
                if(i.name == 'Thursday'){
                    thur.Breakfast.push(element) 
                }
                if(i.name == 'Friday'){
                    fri.Breakfast.push(element) 
                }
                if(i.name == 'Saturday'){
                    sat.Breakfast.push(element) 
                }
                if(i.name == 'Sunday'){
                    sun.Breakfast.push(element) 
                }
                console.log(i.name);
